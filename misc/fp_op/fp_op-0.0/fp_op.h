...
extern void show_errno(0,0,0,0,const char *__s);
// 线程递归文件锁
...
#endif
#define fp_open(path,mode) ({\
	FILE* fp=fopen(path,mode);\
	if(!fp)show_errno(0,"fopen");\
	fp;\
})
#define fp_read(buf,usize,un,fp) ({\
	size_t ret_size=fread(buf,usize,un,fp);\
	if(ret_size<0)show_errno(0,"fread");\
	ret_size;\
})
#define fp_read(buf,usize,un,fp) ({\
	size_t ret_size=fread(buf,usize,un,fp);\
...
#define fp_write(buf,usize,un,fp) ({\
	size_t ret_size=fwrite(buf,usize,un,fp);\
	if(ret_size<0)show_errno(0,"fwrite");\
	ret_size;\
})
#define fp_close(fp) ({\
	int ret=fclose(fp);\
	if(ret<0)show_errno(0,"fclose");\
	ret;\
})
#endif