...
Memory_Pool *memory_pool_create(int size, int increment){
    char *p = NULL;
    char *p_memory = NULL;
    ...
    if (size < 0 || size > MAX_POOL_SIZE){
        printf("memory_pool_create(): Invalid size(%d)\n", size);
        return pool;
    }
    ...
    printf("memory_pool_create: total size: %d, block cnt: %d, block size: %d\n",
