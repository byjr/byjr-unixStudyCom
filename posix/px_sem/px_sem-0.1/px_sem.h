...
#define px_seminit(p_sem,pshared,value) ({\
	...
	if(-1==ret)show_errno(0,"sem_init");\
	ret;\
})
...
#define  px_semgetvalue(p_val,p_sem) ({\
	...
	if(-1==ret)show_errno(0,"sem_getvalue");\
	ret=-1==ret?ret:*p_val;\
})
...
#define px_semwait(p_sem) ({\
	...
	if(-1==ret)show_errno(0,"sem_wait");\
	ret;\
})

#define px_semtrywait(p_sem) ({\
	int ret=sem_trywait(p_sem);\
	if(-1==ret)show_errno(0,"sem_trywait");\
	ret;\
})
...
#define px_semtimedwait(p_sem,tsv) ({\
	...
	do{\
		...
		if(-1==ret)show_errno(0,"sem_timedwait");\
	}while(0);\
	ret;\
})
...
#define px_semdestroy(p_sem) ({\
	...
	if(-1==ret)show_errno(0,"sem_destroy");\
	ret;\
})

#define px_semunlink(path) ({\
	int ret=sem_unlink(path);\
	if(-1==ret)show_errno(0,"sem_unlink");\
	ret;\
})
